...
class MetaTemplate(nn.Module):
    def __init__(self, model_func, n_way, n_support, tracking, use_bn, pretrain, depth, normalize, change_way = True):
        ...
        if isinstance(model_func,str):
            if model_func == 'resnet18':
                ...
                self.feature = resnet18(pretrained=pretrain, tracking=tracking)
                # self.feature = torch.nn.Sequential(*(list(model.children())[:-2]))
            if self.depth:
                self.feature_depth = ResidualNet('ImageNet', 18, 1000, None, tracking=tracking, use_bn=self.use_bn)
                ## will change to 1 channel later
                # self.feature_depth.conv1 = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3, bias=False)
                # import ipdb; ipdb.set_trace()
            ...
            if self.depth:
                self.feature_depth = self.feature
            self.feat_dim   = self.feature.final_feat_dim
        # import ipdb; ipdb.set_trace()
