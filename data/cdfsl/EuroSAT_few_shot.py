...
class SimpleDataset:
    def __init__(self, transform, target_transform=identity, \
        ...
        csv_file = open("filelists/EuroSAT/eurosat_filename_label.csv")
        ...
        for i, row in enumerate(csv_reader):
            if i == 0:
                continue
            self.meta['image_names'].append(os.path.join(configs.data_dir["EuroSAT"], row[1], row[0]))
            labels.append(row[1])
...
class SubDataset:
    ...
    def __getitem__(self,i):
        ...
        img = self.transform(Image.open(self.sub_meta[i]))
