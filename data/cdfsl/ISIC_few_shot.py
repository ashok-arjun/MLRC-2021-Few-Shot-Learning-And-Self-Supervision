...
class CustomDatasetFromImages(Dataset):
    def __init__(self, csv_path= configs.data_dir["ISIC"] + "/ISIC2018_Task3_Training_GroundTruth/ISIC2018_Task3_Training_GroundTruth.csv", \
        image_path =  configs.data_dir["ISIC"] + "/ISIC2018_Task3_Training_Input/"):
        """
        Args:
    ...
    def __getitem__(self, index):
        ...
        # Open image
        temp = Image.open(self.img_path +  single_image_name + ".jpg").convert('RGB')
...
class SimpleDataset:
    def __init__(self, transform, target_transform=identity, \
        ...
        csv_path= configs.data_dir["ISIC"] + "/ISIC2018_Task3_Training_GroundTruth/ISIC2018_Task3_Training_GroundTruth.csv"
        self.image_path =  configs.data_dir["ISIC"] + "/ISIC2018_Task3_Training_Input/"

        self.data_info = pd.read_csv(csv_path, skiprows=[0], header=None)

        # First column contains the image paths
        self.meta['image_names'] = np.asarray(self.data_info.iloc[:, 0])

        self.labels = np.asarray(self.data_info.iloc[:, 1:])
        self.meta['image_labels'] = (self.labels!=0).argmax(axis=1)
    ...
    def __getitem__(self, i):
        ...
        img = Image.open(os.path.join(self.image_path, self.meta['image_names'][i] + ".jpg")).convert('RGB')
...
class SetDataset:
    def __init__(self, batch_size, transform):
        ...
        for i, (label) in enumerate(self.meta["image_labels"]):
            self.sub_meta[label].append(os.path.join(self.image_path,self.meta['image_names'][i]+ ".jpg"))
